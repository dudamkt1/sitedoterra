...
export function TenantSite({
  ...
  function clearSelection() {
    ...
    setSelectedDate("Horários disponíveis");
    setShowSlots(false);
            ...
            {showSlots && (
              <div className="time-slots">
                <div className="flex items-center justify-between gap-2 mb-3">
                  <p className="time-slots-title !mb-0" id="selectedDate">{selectedDate}</p>
            ...
            {showConfirm && (
              <div id="wppConfirm" style={{ marginTop: "1.5rem", padding: "1.5rem", background: "#f0fdf4", borderRadius: 12, border: "1.5px solid #bbf7d0" }}>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button type="button" onClick={clearSelection} className="inline-flex items-center justify-center rounded-full border border-gray-200 bg-white px-4 py-2 text-xs font-semibold text-gray-600 hover:bg-gray-50">Trocar horário</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="insta-grid">
          {[["🌿", "linear-gradient(135deg, #d4e8d4 0%, #a8d5b5 100%)"], ["🍋", "linear-gradient(135deg, #f5e6d0 0%, #e8c87a 100%)"], ["🌸", "linear-gradient(135deg, #c8e6d4 0%, #1D5C3A 100%)"], ["🧴", "linear-gradient(135deg, #fbe8d0 0%, #C4963A 100%)"], ["🌱", "linear-gradient(135deg, #e0f2e8 0%, #4A9E6B 100%)"]].map(([e, g], i) => (
            <div key={i} className="insta-item reveal" style={{ transitionDelay: `${i * 0.1}s` }}>
              ...
              <div className="insta-overlay"><svg viewBox="0 0 24 24" fillRule="evenodd"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073z"/></svg></div>
            </div>
          ))}
        </div>
      </section>
