-- ============================================================================
-- Site público: trial válido conta como assinatura ativa
-- ============================================================================
-- Ativação PAGA cria assinatura `active` com trial_end futuro (3 meses
-- inclusos). Alguns fluxos usam `trialing` com trial_end futuro — que a regra
-- central (effectiveSubscriptionStatus) considera ativo. O RPC público exigia
-- status = 'active' exato, deixando site PAGO como "desativado".
-- Agora aceita `active` OU `trialing` com trial_end no futuro.
-- ============================================================================
    ...
    and (
      ...
      or exists (
        ...
        where su.tenant_id = t.id
          and (
            su.status = 'active'
            or (su.status = 'trialing' and su.trial_end is not null and su.trial_end > now())
          )
      )
    );
$$;
