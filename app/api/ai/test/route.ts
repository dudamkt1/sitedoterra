 ...
 * Usa a chave salva; se `api_key` for enviada (chave recém-digitada ainda não
 * salva), testa com ela para o usuário validar antes de salvar. */
export async function POST(request: Request) {
  ...
  const result = await testProviderConnection(user.id, providerId, apiKey);
